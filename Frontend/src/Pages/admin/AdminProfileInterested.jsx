...
function AdminProfileInterested() {
  const [profileInterests, setProfileInterests] = useState([]);
  const [allProfileInterests, setAllProfileInterests] = useState([]);
  ...
  // Pagination logic
  const totalPages = Math.ceil(profileInterests.length / itemsPerPage);
  const paginatedProfileInterests = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return profileInterests.slice(start, start + itemsPerPage);
  }, [profileInterests, currentPage]);

  useEffect(() => { setCurrentPage(1); }, [profileInterests.length]);

  // Fetch profile interests data
  useEffect(() => {
    const fetchProfileInterests = async () => {
      try {
        const response = await fetch(`${API}/api/admin/profile-interests`);
        if (!response.ok) {
          throw new Error("Failed to fetch profile interests");
        }
        const data = await response.json();
        setProfileInterests(data);
        setAllProfileInterests(data);
      } catch (error) {
  ...
  const handleSearch = async () => {
    try {
      if (searchKey.trim() === "") {
        setProfileInterests(allProfileInterests);
        return;
      }

      const response = await fetch(
        `${API}/api/admin/profile-interests/search?search=${searchKey}`
      );
      if (!response.ok) {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(
        `${API}/api/admin/profile-interests/${selectedInterest.id}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to delete profile interest");
      }

      const updatedInterests = profileInterests.filter(
        (i) => i.id !== selectedInterest.id
      );
      const updatedAllInterests = allProfileInterests.filter(
        (i) => i.id !== selectedInterest.id
      );

      setProfileInterests(updatedInterests);
      setAllProfileInterests(updatedAllInterests);
    ...
    <div className="mt-12">
      ...
      <div className="card bg-white shadow-sm rounded-lg mb-6">
        <div className="card-body p-4">
          <h5 className="text-xl font-semibold mb-3">Profile Interested</h5>
              ...
              <table className="min-w-full table-auto">
                <thead className="bg-gray-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-sm font-medium font-workSans text-gray-700">
                      Interested Profile Name
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium font-workSans text-gray-700">
                      Interested Profile ID
                    </th>
                    <th className="px-4 py-3 text-left text-sm font-medium font-workSans text-gray-700">
                      Actions
                    </th>
                ...
                <tbody className="text-sm">
                  {paginatedProfileInterests.map((interest, index) => (
                    <tr key={interest.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                      <td className="px-4 py-3">{interest.customer_name}</td>
                      <td className="px-4 py-3">{interest.mem_id}</td>
                      <td className="px-4 py-3">{interest.profile_name}</td>
                      <td className="px-4 py-3">{interest.profile_mem_id}</td>
          ...
          {totalPages > 0 && (
            <div className="px-4 py-3 bg-gray-50 border-t flex flex-col md:flex-row items-center justify-between gap-3">
              <p className="text-sm text-gray-500">
                ...
                <span className="font-bold text-gray-700">{Math.min(currentPage * itemsPerPage, profileInterests.length)}</span> of{" "}
                <span className="font-bold text-gray-700">{profileInterests.length}</span> entries
              </p>
...
export default AdminProfileInterested;
